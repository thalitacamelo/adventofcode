...
# Draw of numbers until completion of one row or column on a board.
flag = None
    ...
    for i in range(len(boards)):
        ...
        if number in board_dict:
            ...
            if 5 in row_count or 5 in col_count:
                # If there is at least one complete row or column of marked numbers on a board, break the loop and store the last number drawn and the winning board.
                flag = [number, board_dict]
                break
    if flag is not None:
        break

last_number, winning_board_dict = flag
sum = 0
